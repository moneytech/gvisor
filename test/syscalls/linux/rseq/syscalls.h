...
extern "C" uintptr_t syscall(int number, ...);

static inline void sys_exit_group(int status) { syscall(kExitGroup, status); }
static inline int sys_getpid() { return static_cast<int>(syscall(kGetpid)); }
