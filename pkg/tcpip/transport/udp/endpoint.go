...
func (e *endpoint) Read(addr *tcpip.FullAddress) (buffer.View, tcpip.ControlMessages, *tcpip.Error) {
	...
	return p.data.ToView(), tcpip.ControlMessages{HasTimestamp: true, Timestamp: p.timestamp, HasTOS: e.receiveTOS, TOS: p.tos}, nil
}
...
func (e *endpoint) HandlePacket(r *stack.Route, id stack.TransportEndpointID, pkt tcpip.PacketBuffer) {
	...
	packet.tos = parseNetworkHeaderTos(r.NetProto, pkt.NetworkHeader)
...
// parseNetworkHeaderTos attempts to get the incoming TOS from a provided
// buffer.View of a Network Header.
func parseNetworkHeaderTos(proto tcpip.NetworkProtocolNumber, headerView buffer.View) uint8 {
	if headerView != nil && proto == header.IPv4ProtocolNumber {
		h := header.IPv4(headerView)
		// This packet has already been validated before being passed up the stack.
		tos, _ := h.TOS()
		return tos
	}
	// IPV6 will be implemented when we implement the IPV6_RECVTCLASS
	return stack.DefaultTOS
}
